#!/usr/bin/env python3
"""Overlay CoCoNet-input human (NLF SMPL-H) on the staged color video (debug.mp4).

``run_horefine.py`` builds ``pth['in']`` from two sources:
- human ``smpl_pose`` / ``smpl_t`` / ``betas`` → ``cari4d/nlf-2unidepth/*_params.pkl``
- object ``pose_abs`` → ``cari4d/fp-hy3d3-unidepth/*_all.pkl`` (object only; no human fields)

This script visualizes the human half. Run after ``fp_hy3d_2dir.py`` so ``*_all.pkl``
exists, even though human params are read from nlf-2unidepth.

Open4D-HOI / wild layout:
- intrinsics from ``cari4d/intrinsics.pkl`` or ``cari4d/videos/*.color.pkl``
- gender from ``cari4d/nlf_gender.txt`` when present, else NLF pickle
"""
...
def _gender_str(raw) -> str:
    if isinstance(raw, np.ndarray):
        raw = raw.item() if raw.shape == () else str(raw)
    g = str(raw).strip().lower()
    return g if g in ("male", "female") else "male"


def _read_nlf_gender_txt(cari4d_root: str) -> str | None:
...
def _load_intrinsics(exp_dir: str, kid: int) -> np.ndarray:
    """Match ``stage_exp_open4dhoi.py`` outputs (same as ``run_open4dhoi.sh``)."""
    ...
    raise FileNotFoundError(
        f"no intrinsics under {cari4d}; run stage_exp_open4dhoi.py first"
    )


def _load_nlf(pkl_path: str, kid: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, str]:
    ...
    return poses, betas, trans, _gender_str(d.get("gender", "male"))


def _load_smpl_uv_tensors(device: str = "cuda") -> dict:
    """Fixed SMPL UV template; no BEHAVE seq-name lookup."""
    if not osp.isfile(_SMPL_UV_OBJ):
        raise FileNotFoundError(_SMPL_UV_OBJ)
...
def render_debug(exp_dir: str, kid: int, out_path: str, nlf_pkl: str, batch: int) -> None:
    ...
    if not osp.isfile(nlf_pkl):
        raise FileNotFoundError(nlf_pkl)

    poses, betas, trans, gender = _load_nlf(nlf_pkl, kid)
    gender_txt = _read_nlf_gender_txt(osp.join(exp_dir, "cari4d"))
    if gender_txt is not None:
        gender = gender_txt
    ...
    for start in tqdm(range(0, T, batch), desc="render debug"):
        ...
        color_r, _depth, _xyz = Utils.nvdiff_color_depth_render(
            K_rois, glctx, mesh_tensors, render_size, verts.contiguous()
        )
    ...
    writer.close()
    print(f"wrote {out_path} ({T} frames, gender={gender}, source={nlf_pkl})")
...
def main() -> None:
    ...
    p.add_argument("--nlf_pkl", default="")
    p.add_argument("--batch", type=int, default=32)
    args = p.parse_args()

    exp_dir = osp.abspath(args.exp_dir)
    video_prefix = osp.basename(exp_dir.rstrip("/"))
    nlf_pkl = args.nlf_pkl or osp.join(
        exp_dir, "cari4d", "nlf-2unidepth", f"{video_prefix}_params.pkl"
    )
    out_path = args.out or osp.join(exp_dir, "debug.mp4")
    render_debug(exp_dir, args.kid, out_path, nlf_pkl, args.batch)
