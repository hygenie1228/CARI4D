...
def main() -> None:
    open4dhoi_root = Path(__file__).resolve().parent / "experiments" / "open4dhoi"

    if not open4dhoi_root.exists() or not open4dhoi_root.is_dir():
        print(f"[ERROR] Directory not found: {open4dhoi_root}")
        return
    ...
    if not sample_dirs:
        print(f"[INFO] No sample directories found in: {open4dhoi_root}")
        return
